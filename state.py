...
# Acuan harga tertinggi TETAP — tidak boleh berubah agar skor SMART konsisten
...
# ── Format harga Rupiah ───────────────────────────────────
def fmt_harga(harga):
    """Format angka ke string Rupiah: Rp 9.000.000"""
...
def utility(lp, max_harga=None, mm=None):
    """
    Hitung nilai utilitas tiap kriteria.
    max_harga SELALU pakai MAX_HARGA_REF agar skor konsisten
    dengan perhitungan manual — jangan pakai actual_budget.
    """
    if mm is None:
        mm = get_minmax()
    if max_harga is None:
        max_harga = MAX_HARGA_REF
