...
with tab_view:
    ...
    for i, lp in enumerate(st.session_state.laptops):
        ...
        <tr class="{cls}">
            ...
            <td style="font-weight:600;">{state.fmt_harga(lp['harga'])}</td>
        </tr>"""
    ...
    <table class="spk-table">
        <thead>
            <tr>
                <th class="tc" width="40">No</th><th>Nama / Model Laptop</th><th>Processor</th>
                <th class="tc">Skor Proc</th><th class="tc">RAM</th><th class="tc">Storage</th>
    ...
    with tab_add:
        with st.form("form_add_laptop", clear_on_submit=True):
            ...
            with c2:
                ...
                hg  = st.number_input("Harga Laptop (Rupiah)", 1_000_000, 150_000_000, 10_000_000, step=100_000,
                                      format="%d")
        ...
        if sub:
            if not nm.strip():
                ...
                st.session_state.laptops.append({
                    ...
                    "ram": rm, "storage": st2, "battery": bt, "harga": int(hg)
                })
    ...
    with tab_edit:
        ...
        with st.form("form_edit_laptop"):
            ...
            with c2:
                ...
                hg  = st.number_input("Harga Laptop (Rupiah)", 1_000_000, 150_000_000, int(lp["harga"]),
...
if tab_del:
    with tab_del:
        ...
        <div style="background:#2a0a0a; border:1px solid #7f1d1d; color:#f87171;
            ...
            ⚠️ <b>Peringatan Penting:</b> Anda akan menghapus unit <b>{target_name}</b> secara permanen
            dari basis data sistem SPK ini. Tindakan ini tidak dapat dibatalkan.
