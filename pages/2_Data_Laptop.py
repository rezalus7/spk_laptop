...
with st.sidebar:
    ...
    if st.button("🚪 Keluar Sistem", use_container_width=True, type="secondary"):
        st.session_state.logged_in = False; st.session_state.username = ""; st.session_state.role = ""; st.rerun()
...
if st.session_state.role == "admin":
    tab_view, tab_add, tab_edit, tab_del = st.tabs(["🔍 Lihat Data", "➕ Tambah Laptop", "📝 Edit Data", "❌ Hapus Laptop"])
else:
    tab_view = st.tabs(["🔍 Lihat Data"])[0]; tab_add = tab_edit = tab_del = None
...
with tab_view:
    ...
    for i, lp in enumerate(st.session_state.laptops):
        ...
        <tr class="{cls}">
            ...
            <td class="tc"><span style="background:rgba(59,130,246,.1); color:#60a5fa; padding:2px 8px; border-radius:10px; font-size:12px;">{lp['processor_score']}</span></td>
            <td class="tc">{lp['ram']} GB</td><td class="tc">{lp['storage']} GB</td><td class="tc">{lp['battery']} mAh</td><td style="font-weight:600;">Rp {display_price:,}</td>
        </tr>"""

    st.markdown(f'<div class="spk-table-wrap"><table class="spk-table"><thead><tr><th class="tc" width="40">No</th><th>Nama / Model Laptop</th><th>Processor</th><th class="tc">Skor Proc</th><th class="tc">RAM</th><th class="tc">Storage</th><th class="tc">Baterai</th><th>Harga</th></tr></thead><tbody>{rows}</tbody></table></div>', unsafe_allow_html=True)
    ...
    with tab_add:
        with st.form("form_add_laptop", clear_on_submit=True):
            ...
            with c2:
                ...
                bt = st.number_input("Kapasitas Baterai (mAh)", 1000, 15000, 4000)
        ...
        if sub:
            if not nm.strip(): state.set_flash("warn", "⚠️ Nama laptop tidak boleh kosong."); st.rerun()
            else:
                st.session_state.laptops.append({"nama": nm.strip(), "processor": pr.strip(), "processor_score": ps, "ram": rm, "storage": st2, "battery": bt, "harga": float(hg / 1_000_000)})
                state.set_flash("ok", f'✅ Laptop "{nm.strip()}" berhasil ditambahkan.'); st.rerun()
...
if tab_edit:
    with tab_edit:
        ...
        with st.form("form_edit_laptop"):
            ...
            with c2:
                ram_opts = [4, 8, 12, 16, 32]; r_idx = ram_opts.index(lp["ram"]) if lp["ram"] in ram_opts else 1
                rm = st.selectbox("Kapasitas RAM", ram_opts, index=r_idx, format_func=lambda x: f"{x} GB")
                stor_opts = [128, 256, 512, 1024, 2048]; s_idx = stor_opts.index(lp["storage"]) if lp["storage"] in stor_opts else 2
...
if tab_del:
    with tab_del:
        ...
        idx2 = int(sel2.split(".")[0]) - 1; target_name = st.session_state.laptops[idx2]["nama"]
        st.markdown(f'<div style="background:#2a0a0a; border:1px solid #7f1d1d; color:#f87171; padding:16px; border-radius:12px; margin-bottom:20px; font-size:14px;">⚠️ <b>Peringatan Penting:</b> Anda akan menghapus unit <b>{target_name}</b> secara permanen dari basis data sistem SPK ini. Tindakan ini tidak dapat dibatalkan.</div>', unsafe_allow_html=True)
        if st.button("❌ Hapus Permanen", use_container_width=True, type="primary"):
            st.session_state.laptops.pop(idx2); state.set_flash("ok", f'🗑️ Laptop "{target_name}" berhasil dihapus dari sistem.'); st.rerun()